...
When CBP publishes an ambiguous or unavailable source response, the adapter and collector still fail closed and append nothing. The scheduled collector reports that condition as a visible safe skip rather than a process failure; unexpected collector errors continue to fail the workflow.
