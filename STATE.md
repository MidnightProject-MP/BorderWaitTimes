...
- Delivery: GitHub account `MidnightProject-MP` is authenticated. Remote is `https://github.com/MidnightProject-MP/BorderWaitTimes.git`; local `main` tracks `origin/main`. The archive collector, checks, automation, and first valid observations are included in `36bdebb`; push verification is pending.
