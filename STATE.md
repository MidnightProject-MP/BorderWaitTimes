...
- Verification: `npm run verify:cbp-archive`, `npm run check:cbp`, `npm run check:cbp-archive`, `npm run check:adapter`, all JavaScript syntax checks, `npm run check:browser`, and `git diff --check` pass. Verification accepts the live archive (30 rows across 2 partitions) and detects tampered values, duplicate identities, invalid or out-of-partition timestamps, unsupported enums, schema drift, and mismatched identity hashes in fixtures.
- Delivery: GitHub account `MidnightProject-MP` is authenticated. Remote is `https://github.com/MidnightProject-MP/BorderWaitTimes.git`; local `main` is clean, pushed, and tracks `origin/main` at `656f8b0`. The mobile-first primary journey is included in `0e0602b`; archive verification is included in `656f8b0`. Manual workflow runs `32922082873` and `32922084186` completed successfully against `656f8b0`, including verification before collection.
