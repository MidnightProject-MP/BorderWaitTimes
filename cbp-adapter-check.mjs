...
assert.equal(stale.ports[0].lanes.passengerStandard.reportedDelayMinutes, 60);
assert.equal(stale.ports[0].lanes.passengerStandard.reportedLanesOpen, 3);
assert.equal(fresh.ports[0].portDate, '8/25/2026');
assert.equal(normalizeCbpXml(fixture.replace('<date>8/25/2026</date>', ''), { now }).ports[0].status, 'unknown');
assert.equal(normalizeCbpXml(fixture.replace('<date>8/25/2026</date>', '<date>8/24/2026</date>'), { now }).ports[0].status, 'unknown');
assert.equal(normalizeCbpXml(fixture.replace('<date>8/25/2026</date>', '<date>2/30/2026</date>'), { now }).ports[0].status, 'unknown');
assert.equal(normalizeCbpXml(fixture.replace('2026-8-25', '2026-2-30'), { now }).ports[0].status, 'unknown');
